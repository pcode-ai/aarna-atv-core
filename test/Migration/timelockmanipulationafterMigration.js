...
describe('AFiBase - migration testing', () => {
    ...
    beforeEach(async () => {
        ...
        const AFiTimelock = await ethers.getContractFactory('AFiTimeLockMock');
        ...
        AFiTimelockInstance = await AFiTimelock.deploy(aFiTestTokenInstace.address);
        const payload = [
            [
        ...
        await aFiStorageInstance.setStablesWithdrawalLimit(aTokenConInstance.address, 500000000000000000000n);
        ...
        */
        ...
        // await AFiTimelockInstance.pause();
        await AFiTimelockInstance.setLockDateDetails(lockDates, lockDateFactors);
        await aFiTestTokenInstace.mint(platformWallet.address, 10000);
        await aFiTestTokenInstace.approve(AFiTimelockInstance.address, 10000);

        await AFiTimelockInstance.addAFiToken(parentaTokenConInstance.address);
        await aFiTestTokenInstace.approve(AFiTimelockInstance.address, 1000);
        await AFiTimelockInstance.setCap((1000));
        await AFiTimelockInstance.setBaseRate(100);
        await AFiTimelockInstance.setLockPeriodDetails(lockPeriods, lockFactors);

        console.log("funded account balance usdttttttttt", investorusdtBalance);
    });

    context('Parent base TL manipulation', () => {

        it("Tl manipulation after migration and stake unstake when parent vault is not frozen", async () => {
            let depositAmount = 0;
            ...
            await expect(parentaTokenConInstance.connect(investor1).deposit(
                depositAmount, usdtConInstance.address, false
            )).to.be.reverted;

            //when payment token is not the whitelisted one
            await expect(parentaTokenConInstance.connect(investor1).deposit(
                1000000000, "0xD31a59c85aE9D8edEFeC411D448f90841571b89c", true
            )).to.be.reverted;
            ...
            const totalSupply = await parentaTokenConInstance.totalSupply();
            var afiTokenBal = await parentaTokenConInstance.balanceOf(investor1.address);
            ...
            await parentaTokenConInstance.connect(investor1).pauseUnpauseDeposit(true);
            console.log("stake initates=============");

            await parentaTokenConInstance.pauseWithdraw(true);

            console.log("stake initates=============");
            await parentaTokenConInstance.updateTimeLockContract(AFiTimelockInstance.address);

            afiTokenBal = await parentaTokenConInstance.userLockedAmount(investor1.address);
            console.log(`balance of locked afi Token before stake, ${afiTokenBal}`);

            await AFiTimelockInstance.connect(investor1).stake((1), parentaTokenConInstance.address, 1);

            afiTokenBal = await parentaTokenConInstance.userLockedAmount(investor1.address);
            console.log(`balance of locked afi Token after stake, ${afiTokenBal}`);

            const stakingDetails = await AFiTimelockInstance.stakingDetails(investor1.address, 0);
            ...
            await AFiTimelockInstance.freezeRewardsForAFiToken(parentaTokenConInstance.address);
            ...
            afiTokenBal = await parentaTokenConInstance.userLockedAmount(investor1.address);
            console.log(`balance of locked afi Token before unstake, ${afiTokenBal}`);

            await AFiTimelockInstance.connect(investor1).unstake(0);
        ...
        it("Tl manipulation after migration and stake unstake when parent vault is frozen", async () => {
            let depositAmount = 0;
