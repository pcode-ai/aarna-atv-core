...
describe('AFiBase', () => {
    ...
    before(async () => {
        ...
        const accountToInpersonate1 = "0x6FfC5757DcC7Aa18b02E408E0b6571eCACae906a"

        await hre.network.provider.request({
            method: "hardhat_impersonateAccount",
            params: [accountToInpersonate1],
        });
        signer1 = await ethers.getSigner(accountToInpersonate1)

        // Transfer 10 ETH
        const transferAmount = ethers.utils.parseEther("2.0");
        // Send transaction
        const tx = await signer.sendTransaction({
            to: accountToInpersonate1,
        ...
        result = await aFiFactoryInstance.createAToken("AFiBase", "ATOK", bytesPayload2, [investor1.address, investor2.address], true, aFiStorageInstance.address,
            aFiPassiveRebalanceInstance.address, signer1.address, ["0xdAC17F958D2ee523a2206206994597C13D831ec7", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", "0x6B175474E89094C44Da98b954EedeAC495271d0F"], "0x0000000000000000000000000000000000000000");
        ...
        */
        ...
        await aFiPassiveRebalanceInstance.setDexForTokenPair("0xdAC17F958D2ee523a2206206994597C13D831ec7", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", 1);
        ...
        swapData = ""
    });

    it("deposit check", async () => {
        console.log("check");
        //await aTokenConInstance.approveForOneInch(["0xdAC17F958D2ee523a2206206994597C13D831ec7"],["60000000"])
        await usdtConInstance.connect(signer1).transfer(aTokenConInstance.address, "100000000");
        await aTokenConInstance.connect(signer1).swapfromSelectiveDex(usdtConInstance.address, "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
            60000000, deadline, "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2", 0, swapData);
    ...
    //     await aFiAFiOracleInstance.connect(investor1).cumulativeSwap(swapParams, 0, ["0x" , "0x", "0x" , "0x", "0x"] ,"0x");
    ...
    //         Afterbal, usdtConInstance.address, deadline, returnString, 3
    //     );
