...
describe('AFiBase - migration testing', () => {
    ...
    context('Rebalance scenario testing', () => {
        ...
        it("Rebalance scenario 2 testing after migration in the new vault when itoken is DAI", async () => {
            ...
            await parentaTokenConInstance.connect(investor1).deposit(
                1000000000, usdcConInstance.address, true
            );
            ...
            await parentaTokenConInstance.connect(investor1).deposit(
                1000000000, usdtConInstance.address, true
            );
            ...
            console.log("enforced token transfers---------------");

            await usdtConInstance.connect(investor1).transfer(parentaTokenConInstance.address, 100000000);
            ...
            await parentaTokenConInstance.connect(investor1).withdraw(
                ether(2), usdtConInstance.address, deadline, returnString, true, 1
            );

            await parentaFiAFiOracleInstance.connect(investor1).queueWithdraw(
                parentaTokenConInstance.address, ether(2), usdtConInstance.address
            );

            checkNav = await parentaFiFactoryInstance.getPricePerFullShare(parentaTokenConInstance.address, aFiStorageInstance.address);
            ...
            await usdtConInstance.connect(investor1).transfer(parentaTokenConInstance.address, balInjection);

            checkNav = await parentaFiFactoryInstance.getPricePerFullShare(parentaTokenConInstance.address, aFiStorageInstance.address);
            console.log("check nav - before unstake", `${checkNav}`);

            await parentaFiAFiOracleInstance.connect(investor1).pauseUnpauseQueue(parentaTokenConInstance.address, true);

            poolValue = await parentaFiStorageInstance.calculatePoolInUsd(parentaTokenConInstance.address);
            // getPriceInUSD

            await parentaFiAFiOracleInstance.connect(investor1).unstakeForQueuedWithdrawals(parentaTokenConInstance.address, usdtConInstance.address, deadline, [0, 0, 0, 0, 0], [0, 0, 0], true, 0);
            ...
            balInjection = await usdtConInstance.balanceOf(investor1.address);
            console.log("before redeem usdt bal", balInjection);


            await parentaFiAFiOracleInstance.connect(investor1).redeem(parentaTokenConInstance.address, ["0xdAC17F958D2ee523a2206206994597C13D831ec7"], 0);
