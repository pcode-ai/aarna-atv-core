...
describe('AFiBase', () => {
    ...
    beforeEach(async () => {
        ...
        */
        ...
        console.log("usdtBalance", usdtBalance)
        await usdtConInstance.connect(signer).transfer(investor1.address, "100000000000");
        await usdtConInstance.connect(signer).transfer(investor2.address, "100000000000");
    ...
    context('Basic checks for deposit and withdraw', () => {
        it("withdraw check", async () => {
            ...
            // await aFiAFiOracleInstance.connect(investor1).cumulativeSwap(swapParams);


            var poolAfterCS = await aTokenConInstance.pool();
            console.log("poolAfterCS ***************", `${poolAfterCS}`);
            ...
            var navstorage = await aFiFactoryInstance.getPricePerFullShare(aTokenConInstance.address, aFiStorageInstance.address);
            console.log("Nav from storage after cs", `${navstorage}`);
            ...
            await aTokenConInstance.connect(investor1).deposit(
                5000000000, usdtConInstance.address
            );

            navstorage = await aFiFactoryInstance.getPricePerFullShare(aTokenConInstance.address, aFiStorageInstance.address);
            console.log("Nav from storage after investor 2 first dep", `${navstorage}`);
            console.log("----------------------------------------------2");
            ...
            navstorage = await aFiFactoryInstance.getPricePerFullShare(aTokenConInstance.address, aFiStorageInstance.address);
            console.log("Nav from storage after investor 2 sec dep", `${navstorage}`);

            var afterdepositBal = await aTokenConInstance.balanceOf(investor2.address)
            console.log("after second deposit share", `${afterdepositBal}`)
        });
    });
});