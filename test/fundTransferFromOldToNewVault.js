...
describe('AFiBase - migration testing', () => {
    ...
    beforeEach(async () => {
        ...
        await parentaFiStorageInstance.setStablesWithdrawalLimit(parentaTokenConInstance.address, usdtConInstance.address, 500000000000000000000n);
    ...
    context('Basic checks for deposit and withdraw', () => {
        ...
        it("deposit check from parent to the child contract", async () => {
            let depositAmount = 0;
            ...
            console.log("Fund transfer begins----------------");

            var itokensInParent = await parentaTokenConInstance.getInputToken();
            console.log("iTokens in the parent vault before removal----------------", itokensInParent);

            await parentaFiStorageInstance.setAFiManager(investor1.address);
            await parentaFiStorageInstance.connect(investor1).deletePreDepositedInputToken(parentaTokenConInstance.address, usdtConInstance.address, 0);
            await parentaTokenConInstance.removeFromWhitelist(usdtConInstance.address, usdcConInstance.address, 0, 0);

            usdtBalanceOfVault = await usdtConInstance.balanceOf(parentaTokenConInstance.address);
            console.log("usdt balance of the old vault before fund transfer----------------", usdtBalanceOfVault);

            usdtBalanceOfVault = await usdtConInstance.balanceOf(aTokenConInstanceNew.address);
            console.log("usdt balance of the new vault before fund transfer----------------", usdtBalanceOfVault);

            await parentaTokenConInstance.emergencyWithdraw(usdtConInstance.address, aTokenConInstanceNew.address);

            usdtBalanceOfVault = await usdtConInstance.balanceOf(aTokenConInstanceNew.address);
            console.log("usdt balance of the new vault after fund transfer----------------", usdtBalanceOfVault);

            usdtBalanceOfVault = await usdtConInstance.balanceOf(parentaTokenConInstance.address);
            console.log("usdt balance of the old vault after fund transfer----------------", usdtBalanceOfVault);

            itokensInParent = await parentaTokenConInstance.getInputToken();
            console.log("iTokens in the parent vault after removal----------------", itokensInParent);

            console.log("Fund transfer partially done----------------");

            // await usdtConInstance.connect(investor1).transfer(aTokenConInstansceNew.address, 1980000000);
